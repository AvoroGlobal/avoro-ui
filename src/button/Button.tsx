...
const sizeStyles: Record<ButtonSize, { height: string; paddingX: string; gap: string; fontSize: string }> = {
...
const variantStyles: Record<ButtonVariant, { bg: string; fg: string; border: string; hoverBg: string; activeBg?: string; activeFg?: string }> = {
  primary: {
...
function Spinner() {
  return (
    <svg
      ...
      style={{ animation: "avoro-spin 0.6s linear infinite" }}
      aria-hidden="true"
    >
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const stateStyles: React.CSSProperties = isDisabled
      ? {
    ...
    return (
      ...
      >
        {loading && <Spinner />}
        {!loading && icon && <AIcon name={icon} size={size === "sm" ? 16 : size === "md" ? 20 : 24} />}
        {children}
      </button>
    );
  }
